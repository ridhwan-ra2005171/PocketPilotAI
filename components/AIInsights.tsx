...
import getAIInsights from '../src/app/actions/getAIInsights';
import { generateInsightAnswer } from '../src/app/actions/generateInsightAnswer';
...
const AIInsights = () => {
  ...
  const [aiAnswers, setAiAnswers] = useState<AIAnswer[]>([]);
   ...
   useEffect(() => {
    loadInsights(); // Load insights when the component mounts
  }, []);
  ...
  const loadInsights = async () => {
    ...
    try {
      ...
      console.error('❌ AIInsights: Failed to load insights:', error);
      ...
      <div className='mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-100 dark:border-gray-700'>
        <div className='flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-0'>
          <div className='flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400'>
            ...
            <span className='font-medium text-xs'>Powered by AI</span>
          </div>
          <button
            onClick={loadInsights}
